...
function renderPostsTab(container, tabsHtml) {
    ...
    if (starPosts.length === 0) {
        ...
        sorted.forEach(p => {
            const date = p.ts ? new Date(p.ts).toLocaleDateString('kk-KZ', { day:'numeric', month:'long', year:'numeric' }) : '';
            postsHtml += `
            <div class="post-card">
                ${p.photo
                ...
                <div class="post-card-body">
                    ...
                    ${date ? `<div class="post-card-date">${date}</div>` : ''}
...
async function addStarPost() {
    ...
    await db.ref("/starPosts").push().set({ text, photo, ts: Date.now() });
